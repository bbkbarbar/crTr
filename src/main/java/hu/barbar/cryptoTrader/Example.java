...
public class Example {
	...
	public static void main(String[] args) {
		...
		try {
			...
			KrakenTradeServiceRaw tradeService2 = (KrakenTradeServiceRaw) krakenExchange.getTradeService();
			Map<String, KrakenOpenPosition> openPositions = tradeService2.getOpenPositions();
		    System.out.println(openPositions + "\n\n");
		    
		    int id = 0;
		    Iterator it = openPositions.entrySet().iterator();
			while (it.hasNext()) {
				Map.Entry pair = (Map.Entry) it.next();
				System.out.println("Key[" + id + "]: " + pair.getKey());
				id++;
				
				KrakenOpenPosition openPosition = (KrakenOpenPosition) pair.getValue();
				System.out.println("Fee: " + openPosition.getFee() );
				System.out.println("Fee: " + openPosition.getAssetPair() );
				
				Set<KrakenOrderFlags> flags = openPosition.getOrderFlags();
			}
